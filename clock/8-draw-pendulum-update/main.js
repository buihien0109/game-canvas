...
class Pendulum {
    constructor(x, y, length, radius, beta) {
        ...
        this.T = 0;
        this.delta = -0.02;
        this.angle = this.beta + Math.PI / 2; // pi/2 - beta => pi/2 + beta;
    }
    ...
    update() {
        ...
        this.angle = this.beta * Math.cos(this.T) + Math.PI / 2 // pi/2 - beta => pi/2 + beta;

        this.T += this.delta;

        if (this.angle < Math.PI / 2 - this.beta) {
...
let pendulum = new Pendulum(centerX, centerY, canvas.height / 2, 35, Math.PI / 6);
